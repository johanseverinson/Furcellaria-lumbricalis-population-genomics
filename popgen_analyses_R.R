...
# ----- Ploidy assessments -----
# --- Using HMMploidy ---
# Import data from HMMploidy file
HOME <- "C:/Users/xsevej/OneDrive - University of Gothenburg/Genetics/Steffis pipeline/Analyses 2025/data/HMMploidy results/All samples" # directory
setwd(HOME)

library(tidyverse)

# A lot of data wrangling below to get the results out of the .HMMploidy file
hmm_file <- "bams_nobad_noT14_dedup.HMMploidy"

x <- readLines(hmm_file)
x <- x[nzchar(x)]  # remove empty lines, if any

head(x, 12) # Each sample has 12 rows

for (i in seq_len(min(12, length(x)))) {
  cat(i, ":", x[i], "\n")
}

parse_nums <- function(z) {
  as.numeric(strsplit(trimws(z), "\\s+")[[1]])
}

parse_hmmploidy <- function(file, lines_per_sample = 12) {
  x <- readLines(file)
  x <- x[nzchar(x)]
  
  n_blocks <- length(x) / lines_per_sample
  
  if (n_blocks != floor(n_blocks)) {
    warning("Number of lines is not a multiple of ", lines_per_sample,
            ". Inspect the file manually with readLines().")
  }
  
  blocks <- split(x, ceiling(seq_along(x) / lines_per_sample))
  
  out <- lapply(blocks, function(b) {
    sample_name <- trimws(b[1])
    
    window_start <- parse_nums(b[9])
    window_end   <- parse_nums(b[10])
    ploidy       <- parse_nums(b[12])
    
    tibble(
      sample = sample_name,
      window_index = seq_along(ploidy),
      start = window_start,
      end = window_end,
      ploidy = ploidy
    )
...
hmm <- parse_hmmploidy("bams_nobad_noT14_dedup.HMMploidy") # Outputs data frame with one row per window for each sample.

# Samples are just called sample 1-10, change to name of actual samples using the bam list input into samtools for the mpileup file
# File used with samtools mpileup -b
bam_list_file <- "bams_nobad_noT14.txt"
meta_df <- read.csv("furcellaria_meta.csv")

bam_paths <- readLines(bam_list_file)
bam_paths <- bam_paths[nzchar(bam_paths)]

sample_names <- bam_paths %>%
  basename() %>%
  str_remove("\\.bam$")

# Check that the number of HMMploidy samples matches the BAM list
old_names <- unique(hmm$sample)

stopifnot(length(old_names) == length(sample_names))
...
hmm <- hmm %>%
  left_join(name_map, by = "sample") %>%
  mutate(sample = sample_name) %>%
  select(-sample_name)

# Ploidy calls per sample. This one gives a few weird numbers for some reason. But get correct from next code block.
counts <- hmm %>%
  count(sample, ploidy)

# Proportion of windows assigned to each ploidy for each sample
counts <- hmm %>%
  group_by(sample) %>%
  summarise(
    n_windows = n(),
    n_haploid = sum(ploidy == 1),
    n_diploid = sum(ploidy == 2),
    prop_haploid = mean(ploidy == 1),
    prop_diploid = mean(ploidy == 2))

library(data.table)
fwrite(
  counts,
  file = "HMMploidy_counts.txt",
  sep = "\t",
  col.names = TRUE,
  quote = FALSE)
...
# --- Using nQuire ---
# Import histograms produced by nQuire
histo_dir <- "C:/Users/xsevej/OneDrive - University of Gothenburg/Genetics/Steffis pipeline/Analyses 2025/data/nQuire results/q20 results"
meta_df <- read.csv("furcellaria_meta.csv")
i2p <- meta_df[,c(3:4)]
colnames(i2p) <- c("sample", "pop")# For dividing by population
...
read_nquire_histo <- function(file) {
  h <- read.table(
    file,
    header = FALSE,
    fill = TRUE,
  ...
  h <- h[, 1:2]
  colnames(h) <- c("freq_percent", "count")
  
  h$sample <- basename(file)
  h$sample <- sub("\\.histo\\.txt$", "", h$sample)
  
  h
}

histos <- bind_rows(lapply(files, read_nquire_histo))

# Calculate as proportion of sites
histos_norm <- histos %>%
  group_by(sample) %>%
  mutate(
...
# Add population to each sample
histos_norm <- histos_norm %>%
  mutate(
    sample_id_21 = substr(sample, 1, 21))
...
histos_norm <- histos_norm %>%
  left_join(i2p, by = "sample_id_21")

histos_by_pop <- split(histos_norm, histos_norm$pop)

# Change pop names for paper
histos_norm <- histos_norm %>%
...
# putting populatons in desired order (edit pop names as needed or skip to plot them alphabetically)
histos_norm$pop=factor(histos_norm$pop,levels=c("S1-F","S1-D","S2-F","S2-D","S3-F","S3-D","B1-F","B1-D")) # Along coast

# Check number of positions per sample
histos_norm <- histos_norm %>%
  group_by(sample_id_21) %>%
  mutate(
    num_positions = sum(count, na.rm = TRUE)) # Number of positions are between 1363-16575. Good. For -q 20 its 2085-25311.

write.csv(histos_norm, "histograms_nQuire.csv")

# 1 graph per population overlapping the lines of each sample. Save as 569 x 600 px
ggplot(
  histos_norm,
  aes(
    x = freq_percent,
  ...
  labs(
    x = "Allele frequency (%)",
    y = "Proportion of biallelic positions") +
  theme_classic() +
  theme(
...
# ----- Individual missing rate and depth across SNPs -----
# Setup environment
library(data.table)
...
# Missingness and Depth averaged across SNPs
library(data.table)

# Read the ANGSD depth distribution (from ANGSD -doDepth 1 and -dumpCounts 1)
#
# Rows: individuals, in the same order as the BAM list
# Columns:
#   column 1 = number of SNPs with depth 0
#   column 2 = number of SNPs with depth 1
#   column 3 = number of SNPs with depth 2
#   etc.

depth_sample <- fread(
  "DATA/myresult2.depthSample",
  header = FALSE)
depth_sample <- depth_sample[,-102] # fread adds empty NA column at end for some reason...

# Read the BAM list so that the rows can be assigned sample names:
bam_list <- fread(
  "bams_nobad",
  header = FALSE
)[[1]]

sample_names <- basename(bam_list)

# Remove BAM extension
sample_names <- sub("\\.bam$", "", sample_names)

# Check that the number of rows in `.depthSample` equals the number of individuals:
stopifnot(nrow(depth_sample) == length(sample_names))

# Convert the table to a numeric matrix:
depth_counts <- as.matrix(depth_sample)
storage.mode(depth_counts) <- "numeric"

rownames(depth_counts) <- sample_names

# Remove T-14 rep in accordance with the rest of the script.
depth_counts <- depth_counts[-48,]
sample_names <- sample_names[-48]

# Each column represents a depth category. Because the first column represents depth 0, the depth values are:
depth_values <- 0:(ncol(depth_counts) - 1)
depth_values

# Calculate the total number of retained SNPs represented for each individual:
n_snps <- rowSums(depth_counts)
summary(n_snps) # 18988 across the board. Good.
# The number should normally be identical among individuals because each row is a depth distribution over the same retained sites:

# Calculate individual missingness. Here, an individual is considered missing at a SNP when its depth is zero:
n_missing <- depth_counts[, 1]
missing_rate <- n_missing / n_snps
missing_percent <- 100 * missing_rate
summary(missing_percent)

# Calculate the number and proportion of SNPs with at least one read:
n_observed <- n_snps - n_missing
observed_rate <- n_observed / n_snps
summary(observed_rate)

# Calculate mean depth across all retained SNPs, including zero-depth SNPs:
total_depth <- as.numeric(
  depth_counts %*% depth_values)
mean_depth_all_snps <- total_depth / n_snps

# Mean depth only among SNPs with at least one read:
mean_depth_observed_snps <- total_depth / n_observed

# Combine the statistics into a table:
individual_summary <- data.table(
  sample = sample_names,
  n_filtered_snps = n_snps,
  n_snps_observed = n_observed,
  n_snps_missing = n_missing,
  individual_missing_rate = missing_rate,
  individual_missing_percent = missing_percent,
  mean_depth_across_all_snps = mean_depth_all_snps,
  mean_depth_at_observed_snps = mean_depth_observed_snps)

individual_summary

# Summarize the values across individuals:
summary(individual_summary$individual_missing_rate)
summary(individual_summary$individual_missing_percent)
summary(individual_summary$mean_depth_across_all_snps)

# Create a compact dataset-level summary:
dataset_summary <- data.table(
  statistic = c(
    "Number of retained SNPs",
    "Minimum individual missing rate",
    "Mean individual missing rate",
    "Median individual missing rate",
    "Maximum individual missing rate",
    "Minimum individual mean depth",
    "Mean individual mean depth",
    "Median individual mean depth",
    "Maximum individual mean depth"),
  value = c(
    unique(n_snps)[1],
    min(missing_rate),
    mean(missing_rate),
    median(missing_rate),
    max(missing_rate),
    min(mean_depth_all_snps),
    mean(mean_depth_all_snps),
    median(mean_depth_all_snps),
    max(mean_depth_all_snps)))
...
# genetic distances
IBS=as.matrix(read.table("DATA/myresult2_noT14.ibsMat"))
dim(IBS)
samples <- meta_df 
...
# --- Creating a distribution of (1-IBS) of technical replicates for determining threshold ---
library(tidyverse)

# Inputs
sample_file <- meta_df <- read.csv("DATA/furcellaria_meta.csv")  # one sample ID per line, in same order as ibsMat
sample_file <- sample_file$indv
replicates <- read.csv("DATA/Replicate_groups.csv")

# replicate_file should look like:
# sample_id,replicate_group
# sampleA_lib1,A
# sampleA_lib2,A
# sampleB_lib1,B
# sampleB_lib2,B
...
# If your .ibsMat contains IBS similarity, convert to distance
# If it already contains 1 - IBS distances, set this to FALSE
convert_similarity_to_distance <- FALSE

dist_mat <- if (convert_similarity_to_distance) {
  1 - ibs
} else {
  ibs
}
...
pairwise <- as.data.frame(as.table(dist_mat)) %>%
  dplyr::rename(sample1 = Var1, sample2 = Var2, ibs_distance = Freq) %>%
  filter(as.character(sample1) < as.character(sample2))

# Add replicate-group info
pairwise_reps <- pairwise %>%
  left_join(replicates, by = c("sample1" = "sample_id")) %>%
  dplyr::rename(group1 = replicate_group) %>%
  left_join(replicates, by = c("sample2" = "sample_id")) %>%
  dplyr::rename(group2 = replicate_group)

# Graph of distances between all samples (including technical replicates)
ggplot(pairwise_reps, aes(x = ibs_distance)) +
  geom_histogram(bins = 40, color = "white") +
  #  geom_vline(xintercept = 0.135, linewidth = 1, linetype = "dashed") +
  labs(
    x = "IBS distance (1 - IBS)",
    y = "Number of samples",
    title = "Distribution of IBS distances among samples",
    #    subtitle = paste0("Dashed line = 95th percentile threshold: ", signif(threshold, 4))
  ) +
  theme_bw()

# Keep only technical replicate pairs
tech_rep_pairs <- pairwise_reps %>%
  filter(!is.na(group1), group1 == group2)

# Inspect values
summary(tech_rep_pairs$ibs_distance)

# Define a threshold from the replicate distribution
# Common choices: max replicate distance, 95th percentile, or 99th percentile
threshold_max <- max(tech_rep_pairs$ibs_distance, na.rm = TRUE)
threshold_95  <- quantile(tech_rep_pairs$ibs_distance, 0.95, na.rm = TRUE)
threshold_99  <- quantile(tech_rep_pairs$ibs_distance, 0.99, na.rm = TRUE)

threshold <- threshold_max

# Plot distribution
ggplot(tech_rep_pairs, aes(x = ibs_distance)) +
  geom_histogram(bins = 6, color = "white") +
  #  geom_vline(xintercept = 0.141, linewidth = 1, linetype = "dashed") +
  labs(
    x = "IBS distance",
    y = "Technical replicate pairs",
    title = "",
    #   subtitle = paste0("Dashed line = 95th percentile threshold: ", signif(threshold, 4))
  ) +
  theme_classic() # Save image as 569x220 px

# Save replicate-pair distances
write_csv(tech_rep_pairs, "technical_replicate_IBS_distances.csv")

# Find all sample pairs below the threshold
likely_same_individual <- pairwise %>%
  filter(ibs_distance <= threshold) %>%
  arrange(ibs_distance)

write_csv(likely_same_individual, "pairs_below_IBS_threshold.csv")
...
# For-loop for testing range of cutoffs

# Load IBS matrix and metadata
IBS <- as.matrix(read.table("DATA/myresult2_noT14.ibsMat"))
samples <- meta_df[-48,]
samples <- samples$indv
dimnames(IBS) <- list(samples, samples)
...
# Define cutoffs
cutoffs <- seq(0.140, 0.149, by = 0.001)

# Hierarchical clustering
hc <- hclust(as.dist(IBS), method = "average")
dend <- as.dendrogram(hc)
...
for (cut in cutoffs) {
  ...
  group_labels <- groups[ordered_labels]
  
  # Generate distinct colors
  n_groups <- length(unique(group_labels))
  palette <- qualitative_hcl(n_groups, palette = "Dark 3")
  group_colors <- setNames(palette, sort(unique(group_labels)))
  
  # Color branches
  dend_colored <- color_branches(dend, groupLabels = group_labels, col = group_colors)
  
  # Extract actual colors used for each label
  branch_colors <- get_leaves_branches_col(dend_colored)
  dend_colored <- set(dend_colored, "labels_col", branch_colors)
  dend_colored <- set(dend_colored, "labels_cex", 0.7)
  
  # Plot
  plot(dend_colored, main = paste("Clonal Assignment at Cutoff =", cut, "MLLs = ", n_groups), horiz = TRUE)
  abline(v=cut)
  
  # Save plot as SVG
  svg(filename = paste0("dendrogram_cut_", cut, ".svg"), width = 10, height = 15)
  plot(dend_colored, main = paste("Clonal Assignment at Cutoff =", cut), horiz = TRUE)
  abline(v=cut)
  dev.off()
}
...
# Just insert the correct .ibsmat below. "DATA/myresult2_noclones_cutoff0141.ibsMat" or "DATA/myresult2_noclones.ibsMat"
# ma = as.matrix(read.table("DATA/myresult2_noclones_cutoff0141.ibsMat"))
ma = as.matrix(read.table("DATA/myresult2_noclones.ibsMat"))
...
PCAll <- ggplot(plot_df, aes(x = MDS1, y = -MDS2, color = population, shape = type)) +
...
grid.arrange(PC0141, PCAll, ncol=2) # 569px x 275 px, or 675 x 250 if you include the legends from R.
...
mantel_res # p=0.042 for cutoff 0141, p=0.042 for all samples
...
mantel_res.A # p=0.042 for cutoff 0141, p=0.083 for All samples. rm = 0.836 for All samples.
mantel_res.M # p=0.208 for cutoff 0141, p=0.125 for All samples. rm = 0.825 for All samples.
...
IBD0141 <- ggplot(ibd_df_type, aes(x = geo, y = gen, color = type, shape = type)) +
  geom_smooth(method = "lm", se = TRUE) +
  geom_point(size = 3) +
...
grid.arrange(IBD0141, IBDAll, ncol=2)# Save as 569 x 300
...
# log_files <- list.files(pattern = "myresult2_noclones_ngsadmix_K\\d+.log")
...
LL0141 <- ggplot(summary_data, aes(x = K, y = MeanLogLikelihood)) +
...
qopt_files <- list.files(pattern = "myresult2_noclones_ngsadmix_K\\d+.qopt")
...
# Plot
SW0141 <- ggplot(summary_data, aes(x = K, y = MeanSilhouette)) +
...
# We get the widest width of silhouette for K=3 for cutoff=0.141, while width is greatest for K=2 for cutoff=0.149 and all samples, but log-likelihood better for K=3.
library(gridExtra)
grid.arrange(LLall, LL0141, SWall, SW0141, ncol=2)
ggsave("AdmixK.svg", grid.arrange(LL0141, LLall, SW0141, SWall, ncol=2), units="px", width=569, height=400, dpi=96)
...
inName="myresult2_noclones_ngsadmix_K4.qopt" # name of the input file to plot, output of ngsAdmix or ADMIXTURE run
tbl=read.table("DATA/myresult2_noclones_ngsadmix_K4.qopt") # name of the input file to plot, output of ngsAdmix or ADMIXTURE run
...
tbl$pop=factor(tbl$pop,levels=c("S1-F","S1-D","S2-F","S2-D","S3-F","S3-D","B1-F","B1-D")) # Along coast
...
# --- Admixture of each site ---

# Will do set of all graphs for each cutoff. Could do a for loop but....
par(mfrow=c(3,1))

# assembling the input table
inName="myresult4_noclones_cutoff0141_ngsadmix_K3.qopt" # name of the input file to plot, output of ngsAdmix or ADMIXTURE run
tbl=read.table("DATA/A_cutoff0141_ngsadmix_K3.qopt") # name of the input file to plot, output of ngsAdmix or ADMIXTURE run
pops=i2p # Tab-delimited table containing individual assignments to populations; must be in the same order as samples in the bam list or vcf file.
npops=as.numeric(sub(".+(\\d+)\\..+","\\1",inName))

# If doing individual sites, change i2p to only include that site
i2p <- subset(i2p, i2p$site == "A")
...
# putting populaitons in desired order (edit pop names as needed or skip to plot them alphabetically)
# tbl$pop=factor(tbl$pop,levels=c("K-FL-A","N-FL-A","R-FL-A","A-FL-A","K-FL-M","N-FL-M","R-FL-M","A-FL-M")) # Attached followed by drifting

ords=plotAdmixture(data=tbl, npops=npops, grouping.method="distance", vshift=-0.05, cex=0.92)
...
# --- Make ADMIXTURE graph for each site with K=2 for paper ---
# loading individual to population correspondences
meta_df <- read.csv("DATA/furcellaria_meta_noclones_cutoff0141.csv")
meta_df <- read.csv("DATA/furcellaria_meta_noclones.csv")
i2p <- meta_df[-c(1,2)] # Tab-delimited table containing individual assignments to populations; must be in the same order as samples in the bam list or vcf file.
...
for(i in c("K", "N", "R", "A")){
  ...
  tbl <- read.table(paste0("DATA/",i,"_ngsadmix_K2.qopt")) # name of the input file to plot, output of ngsAdmix
...
grid.arrange(man0141, PC0141, ncol = 2) # Save as 569px x 150px
grid.arrange(manAll, PCAll, ncol = 2) # Save as 569px x 150px


# With chromosomes on x-axis
library(ggplot2)
library(dplyr)
library(data.table)

# Load BayeScan output
bayescan <- read.table("DATA/best.bayescan.cutoff_fst.txt", header = TRUE)

# Load VCF file to get chromosome and position info
vcf <- fread("DATA/myresult4_noclones_cutoff0141.vcf", skip = "#CHROM", select = c(1,2), col.names = c("CHROM", "POS"))

# Combine BayeScan output with VCF info
bayescan$CHROM <- vcf$CHROM
bayescan$POS <- vcf$POS

# Handle extreme Q-values
bayescan$qval[bayescan$qval <= 0.0001] <- 0.0001
bayescan$log10q <- -log10(bayescan$qval)
bayescan$significant <- bayescan$qval < 0.01
...
# Ensure CHROM is a factor with correct order BEFORE any grouping
chrom_order <- mixedsort(unique(bayescan$CHROM))
bayescan$CHROM <- factor(bayescan$CHROM, levels = chrom_order, ordered = TRUE)
bayescan <- bayescan %>%
  arrange(CHROM, POS)

# Calculate offsets
chr_offsets <- bayescan %>%
  group_by(CHROM) %>%
...
# Manhattan plot with alternating background
ggplot(bayescan, aes(x = cum_pos, y = fst)) +
  geom_rect(data = axis_df, aes(xmin = center - 1e6, xmax = center + 1e6, ymin = -Inf, ymax = Inf),
            fill = rep(c("white", "gray95"), length.out = nrow(axis_df)), inherit.aes = FALSE) +
  ...
  theme_minimal() +
  labs(title = "Manhattan Plot of FST from BayeScan",
       x = "Chromosome",
       y = "FST",
       color = "Significant (Q < 0.01)") +
...
pca_df$type <- meta_df$type

# Define custom shapes
custom_shapes <- c("A" = 17, "M" = 16)

# Plot with color by population
PCA1 <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Population, shape = type)) +
...
# Plot with color by population
PCA2 <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Population, shape = type)) +
  geom_point(alpha = 1, cex = 3) +
  scale_shape_manual(values = custom_shapes)+
  labs(title = "",
       x = "PC1 (39.4%)",
...
write.table(outliers[,c(4,5)], "diversifying_PCAngsd_cutoff0141.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

# --- Make supplementary PCAs which compare MLLs of genotype likelihoods to hard calls ---
# We already make a PCA above (PCA2) which is of the MLLs based on genotype likelihoods.
# For hard calls we convert the beagle file to a vcf file and use that.
# --- Check if vcf file produces similar results as we get using IBS distance matrix from ANGSD ---

# Files
vcf_file  <- "/For Marlenes teaching/myresult2_cutoff0141.vcf"
meta_file <- "/For Marlenes teaching/furcellaria_meta_noclones_cutoff0141.csv"
...
#  Read data 
vcf  <- read.vcfR(paste0(DATA, vcf_file), verbose = FALSE)
meta <- fread(paste0(DATA, meta_file))
meta <- as.data.frame(meta)
sample_col <- "bam"      # sample ID column in metadata
pop_col    <- "population"  # population column in metadata

#  Extract genotype matrix 
gt <- vcf@gt[, -1]  # drop FORMAT column
samples <- colnames(gt)

# Convert GT strings to ALT allele counts (0/1/2)
...
# Variants x samples → numeric
X <- apply(gt, 2, gt_to_altcount)
X <- t(X)                    # samples x variants
rownames(X) <- samples

# Mean impute missing values 
variant_means <- colMeans(X, na.rm = TRUE)
na_idx <- which(is.na(X), arr.ind = TRUE)
X[na_idx] <- variant_means[na_idx[, 2]]

# PCA 
pca <- prcomp(X, center = TRUE, scale. = FALSE)
pve <- (pca$sdev^2) / sum(pca$sdev^2) * 100
print(pve) # Look at % explained per PC

scores <- as.data.frame(pca$x)
scores$sample <- rownames(scores)

# Join metadata 
scores <- merge(scores, meta, by.x = "sample", by.y = sample_col, all.x = TRUE)

# Define custom shapes
custom_shapes <- c("A" = 17, "D" = 16)

# Plot, warning just because some populations not enough points to draw ellipse with stat_ellipse
PCA_vcf <- ggplot(scores, aes(-PC1, -PC2, color = population, shape = type)) +
  geom_point(alpha = 1, cex = 3) +
  scale_shape_manual(values = custom_shapes) +
  theme_classic() +
  theme(legend.position = "")+
...
grid.arrange(PCA2, PCA_vcf, ncol=2) # Save as 700 x 300 px to fit the legends.
...
plot_grid(
  ...
  nrow = 1,
  align = "hv",
  axis = "tblr") # Save as 569 x 270 px to fit the legends.


# --- Selective sweep of all samples at each site ---
# Load selection statistics from PCAngsd
setwd(DATA)
# selection_stats <- npyLoad('pcangsd_K_cutoff0141_sel.selection.npy')
selection_stats <- fread('pcangsd_R_sel.selection')
snp_metadata <- fread('myresult5_R.mafs.gz')
sites <- read.csv('pcangsd_R_sel.sites', header=FALSE)
cov_matrix <- read.table("pcangsd_R_sel.cov")
setwd(HOME)
selection_stats <- as.data.frame(selection_stats)
meta_df <- read.csv("DATA/furcellaria_meta_noclones.csv")
meta_df_site <- subset(meta_df, meta_df$site=="R")
...
process_site_all <- function(site_name) {
  # Load data for this site
  selection_stats <- fread(file.path(DATA, paste0("pcangsd_", site_name, "_sel.selection")))
  snp_metadata <- fread(file.path(DATA, paste0("myresult5_", site_name, ".mafs.gz")))
  sites <- read.csv(file.path(DATA, paste0("pcangsd_", site_name, "_sel.sites")), header = FALSE)
  cov_matrix <- read.table(file.path(DATA, paste0("pcangsd_", site_name, "_sel.cov")))
  # Convert to data frame
  selection_stats <- as.data.frame(selection_stats)
  # Meta info
  meta_df <- read.csv(file.path(DATA, "furcellaria_meta_noclones.csv"))
...
# Run for all sites
outlier_list <- lapply(sites_to_analyze, process_site_all)
...
write.table(outlier_list$A[,c(1,2)], "diversifying_PCAngsd_A.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
# Find annotated loci found by several analyses
df <- read.csv("DATA/AnnotatedOutliers.csv")

library(dplyr)

multi_analysis <- df %>%
  distinct(Code, Analysis) %>%        # remove duplicates if any
  group_by(Code) %>%
  filter(n() > 1) %>%                  # keep genes in >1 analysis
  ungroup()

multi_analysis # 34 annotated loci found in more than 1 analysis

# One row per loci with which analyses it was found in
multi_analysis_summary <- df %>%
  distinct(Code, Analysis) %>% 
  group_by(Code) %>%
  summarise(
    n_analyses = n(),
    analyses_found_in = paste(sort(unique(Analysis)), collapse = "; "),
    .groups = "drop"
  ) %>%
  filter(n_analyses > 1)
...
tbl <- read.table("DATA/pcangsd_myresult2_noclones.inbreed.samples")
pops <- read.csv("DATA/furcellaria_meta_noclones.csv")
pops <- pops[,-c(1,2)]
# names(pops)=c("ind", "bam", "pop", "site", "type")
names(pops)=c("bam", "pop", "site", "type")
tbl <- cbind(tbl,pops)
...
glf <- read.table("DATA/myresult2_noclones.beagle.gz", header=TRUE)[,-c(1:3)]
...
# Compare against heterozygosity calculate on both varying and non-varying sites (mybams.het)
tbl <- read.table("DATA/mybams.het")
colnames(tbl) <- c("bamfile", "sitesPassingFilters", "heterozygosity")
tbl <- cbind(tbl, i2p)

boxplot(heterozygosity~pop, data=tbl, ylab="Heterozygosity H")

# Compare heterozygosity on only varying sites against both varying and non-varying
tbl$diff <- tSFS$heterozygosity-tbl$heterozygosity
par(mfrow=c(2,2))
boxplot(diff~pop, data=tbl, ylab="Heterozygosity Diff")
boxplot(heterozygosity~pop, data=tSFS, ylab="Heterozygosity Varying")
boxplot(heterozygosity~pop, data=tbl, ylab="Heterozygosity + non-varying")
par(mfrow=c(1,1))
...
df <- read_csv("DATA/pestPG_allsamples.csv")
...
# For all samples without the MLL stats
# Output table for values of all samples.
output <- cbind(summary_H$mean, summary_Fis$mean, summary_pi$mean_pi, summary_pi_nSite$mean_pi, summary_He$mean, summary_Fis$pop)
colnames(output) <- c("Ho", "Fis", "pi_chrom", "pi_site", "He", "pop")
output
write.csv(output, paste0(DATA, "MLLstats_allSamples.csv"))


# ----- Single locust Fis distributions per population -----
# Import data
HOME <- "C:/Users/xsevej/OneDrive - University of Gothenburg/Genetics/Steffis pipeline/Analyses 2025" # directory
DATA <- paste0(HOME, "/data/") # data folder
RESULT <- paste0(HOME, "/results/") #result folder
...
x <- read.table("DATA/inbreed_sites_AA_all.inbreed.sites", header = FALSE)
head(x)
names(x) <- "Fis"
...
# For all samples
files <- c(
  S1_F    = "DATA/inbreed_sites_KA_all.inbreed.sites",
  S1_D = "DATA/inbreed_sites_KM_all.inbreed.sites",
  S2_F    = "DATA/inbreed_sites_NA_all.inbreed.sites",
  S2_D = "DATA/inbreed_sites_NM_all.inbreed.sites",
  S3_F    = "DATA/inbreed_sites_RA_all.inbreed.sites",
  S3_D = "DATA/inbreed_sites_RM_all.inbreed.sites",
  B1_F    = "DATA/inbreed_sites_AA_all.inbreed.sites",
  B1_D = "DATA/inbreed_sites_AM_all.inbreed.sites")

# For MLLs
files <- c(
  S1_F    = "DATA/inbreed_sites_KA_cutoff0141.inbreed.sites",
  S1_D = "DATA/inbreed_sites_KM_cutoff0141.inbreed.sites",
  S2_F    = "DATA/inbreed_sites_NA_cutoff0141.inbreed.sites",
  S2_D = "DATA/inbreed_sites_NM_cutoff0141.inbreed.sites",
  S3_F    = "DATA/inbreed_sites_RA_cutoff0141.inbreed.sites",
  S3_D = "DATA/inbreed_sites_RM_cutoff0141.inbreed.sites",
  B1_F    = "DATA/inbreed_sites_AA_cutoff0141.inbreed.sites",
  B1_D = "DATA/inbreed_sites_AM_cutoff0141.inbreed.sites")
...
head(fis_data)
names(fis_data) <- c("Fis", "population")

# Sort order of factor
fis_data$population <- factor(fis_data$population, levels = c("S1_F", "S1_D", "S2_F", "S2_D", "S3_F", "S3_D", "B1_F", "B1_D"))

ggplot(fis_data, aes(x = population, y = Fis)) +
...
#  geom_jitter(width = 0.1, size = 3, alpha = 0.9, color = custom_colors, shape = shapes)+
  ...
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1)) # Export as 569x250 px. Will add colors later.

# Large peak at Fis = 1 for S1-Fixed. Check values
S1_F <- subset(fis_data, fis_data$population=="S1_F")
summary(S1_F$Fis)

sum(S1_F$Fis == 1, na.rm = TRUE)
mean(S1_F$Fis == 1, na.rm = TRUE)
...
fis_data %>%
  ...
  summarise(
    n_loci = n(),
    prop_Fis_1 = mean(Fis == 1, na.rm = TRUE),
    prop_positive = mean(Fis > 0, na.rm = TRUE),
    prop_negative = mean(Fis < 0, na.rm = TRUE),
    median = median(Fis, na.rm = TRUE)
  )

# Check Fis against MAF
mafs <- read.table(
  gzfile("DATA/KA_all.mafs.gz"),
  header = TRUE)

head(mafs)
names(mafs)

# Only retain sites which were kept by PCAngsd
beagle_sites <- read.table(
  "DATA/inbreed_sites_KA_all.sites",
  header = FALSE,
  stringsAsFactors = FALSE)

head(beagle_sites)

used_sites <- subset(beagle_sites, beagle_sites$V1=="1")
used_sites$row_R <- rownames(used_sites)

nrow(used_sites)
nrow(S1_F)

# Keep corresponding rows in mafs file
mafs_used <- mafs[used_sites$row_R, ]
...
mafs_used$Fis <- S1_F$Fis

ggplot(mafs_used, aes(x = MAF, y = Fis)) +
...
summary(mafs_used$MAF[mafs_used$Fis > 0.95])
summary(mafs_used$MAF[mafs_used$Fis < 0])
...
df <- read.table("DATA/Fis_Ho_He_pi_allSamples.csv", header=TRUE, sep=",")
...
# Run the test
run_lm_test(df=df, variable_name="R")
...
# Customize B
plot_list$B <- ggplot(df, aes(x = .data[["type"]], y = .data[["B"]])) +
  geom_boxplot(outlier.shape = NA, alpha = 0.2) +
  geom_jitter(width = 0.1, size = 3, alpha = 0.9, color = custom_colors, shape = shapes) +
  theme_classic() +
  labs(x = "", y = "Pareto's β", color = "Site")+
  scale_y_continuous(limits=c(-0.1, 3.2))+
  scale_colour_manual(values="black")

ggarrange(plotlist = plot_list, nrow = 3, ncol = 2) # Save as 569 x 350 px (if Paretos B is included do 569x525 px)
...
# Create a data frame for plotting
Fis_data <- data.frame(
  pop = c(summary_Fis_cutoff0141$pop, summary_Fis_alldata$pop),
  mean = c(summary_Fis_cutoff0141$mean, summary_Fis_alldata$mean),
  se = c(summary_Fis_cutoff0141$se, summary_Fis_alldata$se),
  cutoff = c(rep("cutoff=0141", 8), rep("All samples", 8)),
  MLL = c(MLLs0141$MLL, MLLsalldata$MLL)
)

# Colors for plot
custom_colors <- c(
  "B1-F" = "#ffbb78",  # Light Blue
  "B1-D" = "#e6550d",  # Dark Blue
  "S1-F" = "#aec7e8",  # Light Orange
  "S1-D" = "#084594",  # Dark Orange
  "S2-F" = "#98df8a",  # Light Green
  "S2-D" = "#006d2c",  # Dark Green
  "S3-F" = "#c5b0d5",   # Light Purple
  "S3-D" = "#6a3d9a")   # Dark Purple
...
Fis_data$pop=factor(Fis_data$pop,levels=c("S1-F","S1-D","S2-F","S2-D","S3-F","S3-D","B1-F","B1-D")) # Along coast
...
ggplot(Fis_data, aes(x=pop, y=mean, fill=cutoff))+
  ...
  labs(x='', y='Fis')+
  scale_fill_manual(name='Clone correction', values=c('cutoff=0141'='#8DD2C7','All samples'='#BEB9D9'))+
  theme_classic()+
  theme(legend.title=element_blank())
...
# Read in the high-res version of the european map
...
# Map with sweden and zoomed coasts
ggsave("map.svg", map, units="px", width = 637, height = 500, dpi = 300)

ggsave("kartabasLiten.svg", grid.arrange(swemap, zoomed, ncol=2), units="px", width=637, height=317, dpi=96)
ggsave("kartabasStor.svg", grid.arrange(swemap, zoomed, ncol=2), units="px", width=637, height=500, dpi=96)
