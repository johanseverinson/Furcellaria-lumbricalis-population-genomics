...
# We decided to move forward with cutoff=0.141. Re-run ANGSD with the bam-list with just one individual per MLL after cutoffs.
# Bam lists are bams-cutoff0141. We adjust minInd to 80% of MLLs.
# Run this once without -setMaxDepth in order to be able to calculate mean depth + 4*SD, and then rerun with this filter on. 
# Initial runs showed that mean depth + 4*SD = 9456.549 for cutoff = 0.141. Run ANGSD again with -setMaxDepth filter on.
> angsdNoClones.sh
echo '#!/bin/bash -l
#SBATCH -A naiss2025-22-1128
#SBATCH -p shared
...
# ------------ Ploidy investigation of each sample ------------

# ------ Using HMMploidy (software using hidden markov model to estimate ploidy, adapted for low sequencing depth by using genotype likelihoods, by Soraggi et al. (including Matteo Fumagalli).
git clone https://github.com/SamueleSoraggi/HMMploidy.git

# Employs workflow of: BAM files -> Genotype_Likelihoods.py -> .genolikes files -> HMMploidy.R -> ploidy inference

# Test on a few samples (K-FL-M samples)
# Have bamlist KM.bamlist with all K-M samples + technical replicate
# Create genotype likelihood files. Took ~4 minutes per BAM file using salloc n=1.
python3 ~/bin/Genotype_Likelihoods.py KM.bamlist -p ploidyFile -m 0.2 -dp 10 -s 1 -r cdh_alltags_cc.fasta

# May have to unzip the genolikes.gz files
gunzip *.genolikes.gz

# Create list of genolikes files
ls *.genolikes | sed 's/\.genolikes$//' > genolikes.filelist

# Run HMMploidy. Took >10 sec per sample.
Rscript ~/bin/HMMploidy.R fileList=genolikes.filelist wind=10000 maxPloidy=2

# Better to run this analysis from a mpileup file as then we can set filters using samtools to only include high quality reads and mappings.
samtools mpileup -f cdh_alltags_cc.fasta -b KM.bamlist -q 20 -Q 20 -d 1000 > KM.mpileup

# Create genotype likelihood files. Add the KM.mpileup to an empty text file list (KM.filelist). Removing depth filter since we have that as part of the mpileup.
python3 ~/bin/Genotype_Likelihoods.py KM.filelist -p ploidyFile -m 0.2 -s 1

gunzip *.genolikes.gz

ls *.genolikes | sed 's/\.genolikes$//' > genolikes.filelist

# Added minInd=2 and increased read window following Dunning et al. 2022 (DOI: 10.1111/mec.16691)
Rscript ~/bin/HMMploidy.R fileList=genolikes.filelist wind=100000 maxPloidy=2 minInd=2

# Entire process for all samples as sbatch command. This was very large. Ended up splitting it up into separate steps.
echo '#!/bin/bash -l
#SBATCH -A naiss2025-22-1128
#SBATCH -p shared
#SBATCH --mem 36G
#SBATCH -J HMMploidyAll
#SBATCH -o HMMploidyAll.%J.out
#SBATCH -e HMMploidyAll.%J.err
#SBATCH -t 24:00:00
#SBATCH --mail-type=ALL
#SBATCH --mail-user=johan.severinson@gu.se
...
export HMMPLOIDY_DIR=/cfs/klemming/home/j/jseve/bin
export PYTHONPATH="$HMMPLOIDY_DIR:$PYTHONPATH"

samtools mpileup -f cdh_alltags_cc.fasta -b bams_nobad_noT14 -q 20 -Q 20 -d 1000 > bams_nobad_noT14.mpileup

bgzip bams_nobad_noT14.mpileup

echo bams_nobad_noT14.mpileup.gz > bams_nobad_noT14.filelist

python3 "$HMMPLOIDY_DIR/Genotype_Likelihoods.py" bams_nobad_noT14.filelist -p ploidyFile -m 0.2 -s 1

gunzip *.genolikes.gz

ls *.genolikes | sed 's/\.genolikes$//' > genolikes.filelist

Rscript "$HMMPLOIDY_DIR/HMMploidy.R" fileList=genolikes.filelist wind=100000 maxPloidy=2 minInd=2' > HMMploidyAll.sh

# Some samples were a bit low depth across windows. Will re-run analysis on the bam files of these samples with less strict filters to confirm results.
python3 "$HMMPLOIDY_DIR/Genotype_Likelihoods.py" bams_lowdepth -p ploidyFile -m 0.2 -dp 2 -s 1 -r cdh_alltags_cc.fasta

gunzip *.genolikes.gz

# Change name in genolikes.filelist!
ls YC*.genolikes | sed 's/\.genolikes$//' > genolikes.filelist

Rscript ~/bin/HMMploidy.R fileList=genolikes.filelist wind=100000 maxPloidy=2 minInd=1


# ------ Using nQuire -----
# Also want to try using nQuire on bam files. Will try to install on cluster.
# Following instructions at https://github.com/clwgg/nQuire and moving the created nQuire file to folder with my bam files. DONE

# nQuire can filter for minimum frequency (-f, default is 0.2), mapping quality (-q, default is 1) and minimum coverage (-c, default is 10).
# Says especially mapping quality and coverage are important to assess. Will try different runs and evaluate results.

# Will try for 1 bam file before doing several. Here are the general commands for running nQuire:
nQuire create -b input.bam -o base # Create bin file with base frequencies
nQuire histo base.bin # For making histogram. First column is frequency in % (20-80% since default is minimum frequency = 0.2), 2nd is counts and third represents the count value.
nQuire view base.bin # For inspecting coverage and base counts

# Often there is a lot of noise in base frequencies from mismappings and highly repetetive genomes. Can be handles with strict mapping quality filter, and can use command below:
nQuire denoise base.bin -o base_denoised

# For assessing ploidy level (major analysis step). Output is is 8 tab-separated columns, where column 2 is free model max log-likelihood, column 3 is diploid max log-likelihood and column 6 is delta log-likelihood.
nQuire lrdmodel base.bin # This is more for when you have more possible ploidies than haploid/diploid. Will not use in my case.

# Trying sample A-FL-M-13 since I believe this to be a tetrasporophyte based on microscopy.
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -o A-FL-M-13-nQuire-default
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-default.bin # Inspect the histogram: Is noisy but peaks at either end (20% and 80%) and a big peak at 50%

# Will try denoising using function and then test filters
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-default.bin -o A-FL-M-13-nQuire-default_denoised.bin # kept ~36% of bases.
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-default_denoised.bin # # View new histogram: Is quite similar but peaks are more clear.

# Will try different filters now. Will keep minimum frequency at 0.2 but will increase mapping quality to 30 since thats what we use for ANGSD and still get good number of SNPs.
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -q 30 -o A-FL-M-13-nQuire-mapQ30
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30.bin # Still quite noisy, will try denoise.
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-mapQ30.bin -o A-FL-M-13-nQuire-mapQ30_denoised # Kept ~39%
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30_denoised.bin # Middle peak more clear

# Will increase depth filter to 30
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -q 30 -c 30 -o A-FL-M-13-nQuire-mapQ30C30
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C30.bin # Much less spiky but also much fewer positions. Shape is still the same but 50% peak is smaller.
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-mapQ30C30.bin -o A-FL-M-13-nQuire-mapQ30C30_denoised # Kept 47%, so 502 positions left.
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C30_denoised.bin # Denoise
~/bin/nQuire/nQuire histotest A-FL-M-13-nQuire-mapQ30C30_denoised.bin

# Will reduce depth filter down to 20 since we got very few positions with 30
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -q 30 -c 20 -o A-FL-M-13-nQuire-mapQ30C20
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C20.bin # Might be worst one yet haha
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-mapQ30C20.bin -o A-FL-M-13-nQuire-mapQ30C20_denoised # Kept 39%, so 703 positions left.
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C20_denoised.bin

# Will greatly recude depth filter to c=4
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -q 30 -c 8 -o A-FL-M-13-nQuire-mapQ30C4
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C4.bin # We now also get peaks at 25 and 75%, in addition to the 50%.
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-mapQ30C4.bin -o A-FL-M-13-nQuire-mapQ30C4_denoised # Kept 54%, so 12919 positions left.
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ30C4_denoised.bin # Similar to with noise
~/bin/nQuire/nQuire histotest A-FL-M-13-nQuire-mapQ30C4_denoised.bin

# Will try with a quality filter of 20 (since thats what we use for HMMploidy)
~/bin/nQuire/nQuire create -b YC-4275-A-FL-M-13_S93_R1.rg.bam -q 20 -o A-FL-M-13-nQuire-mapQ20
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ20.bin # Still quite noisy, will try denoise.
~/bin/nQuire/nQuire denoise A-FL-M-13-nQuire-mapQ20.bin -o A-FL-M-13-nQuire-mapQ20_denoised # Kept ~37% 2495 reads left
~/bin/nQuire/nQuire histo A-FL-M-13-nQuire-mapQ20_denoised.bin # Middle peak more clear

# For nQuire the signal is basically the same no matter the filters. The best outcome seems to be using -q 20 but keeping the coverage at default. 
# This netted me plenty of positions even after denoising and clear peaks. Using -q 20 also makes it consistent between analyses.

# Make job for getting the histo outputs for each samples and technical replicate (we already have list of all bams in file "bams")
>nQuireAllSamples.sh
mkdir nQuire_results
...
#SBATCH -n 10
#SBATCH -J nQuireFL
#SBATCH -o nQuireFL.%J.out
#SBATCH -e nQuireFL.%J.err
#SBATCH -t 6:00:00
#SBATCH --mail-type=ALL
#SBATCH --mail-user=johan.severinson@gu.se
#SBATCH --array=1-152

NQUIRE=~/bin/nQuire/nQuire
BAM_LIST=/cfs/klemming/projects/supr/naiss2023-23-410/fastqsconcat_2025/bams_nobad_noT14
OUT_DIR=/cfs/klemming/projects/supr/naiss2023-23-410/fastqsconcat_2025/nQuire_results_q20
...
"$NQUIRE" create \
    -b "$bam" \
    -o "$OUT_DIR/${sample}_q20_c10" \
    -q 20 \

"$NQUIRE" denoise \
    "$OUT_DIR/${sample}_q20_c10.bin" \
    -o "$OUT_DIR/${sample}_q20_c10_denoised"

"$NQUIRE" lrdmodel \
    "$OUT_DIR/${sample}_q20_c10_denoised.bin" \
    > "$OUT_DIR/${sample}_q20_c10.nquire.txt"

"$NQUIRE" histo \
    "$OUT_DIR/${sample}_q20_c10_denoised.bin" \
    > "$OUT_DIR/${sample}_q20_c10.histo.txt"' > nQuireAllSamples.sh


# ------------ Population structure using ngsAdmix ------------

# Run admixture using ngsAdmix for MLLs and all samples
> NGSadmix.sh
echo '#!/bin/bash -l
#SBATCH -A naiss2025-22-1128
#SBATCH -p shared
...
NGSadmix -likes myresult2_noclones.beagle.gz -K 2 -o myresult2_noclones_ngsadmix_K2 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 3 -o myresult2_noclones_ngsadmix_K3 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 4 -o myresult2_noclones_ngsadmix_K4 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 5 -o myresult2_noclones_ngsadmix_K5 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 6 -o myresult2_noclones_ngsadmix_K6 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 7 -o myresult2_noclones_ngsadmix_K7 -P 4
NGSadmix -likes myresult2_noclones.beagle.gz -K 8 -o myresult2_noclones_ngsadmix_K8 -P 4' > NGSadmix.sh
...
angsd -b A.bamlist.cutoff0141.txt -GL 1 $FILTERS1 $TODO -P 1 -out A_cutoff0141 -nThreads 20
NGSadmix -likes A_cutoff0141.beagle.gz -K 2 -o A_cutoff0141_ngsadmix_K2 -P 4

angsd -b K.bamlist.cutoff0141.txt -GL 1 $FILTERS2 $TODO -P 1 -out K_cutoff0141 -nThreads 20
NGSadmix -likes K_cutoff0141.beagle.gz -K 2 -o K_cutoff0141_ngsadmix_K2 -P 4

angsd -b N.bamlist.cutoff0141.txt -GL 1 $FILTERS3 $TODO -P 1 -out N_cutoff0141 -nThreads 20
NGSadmix -likes N_cutoff0141.beagle.gz -K 2 -o N_cutoff0141_ngsadmix_K2 -P 4

angsd -b R.bamlist.cutoff0141.txt -GL 1 $FILTERS4 $TODO -P 1 -out R_cutoff0141 -nThreads 20
NGSadmix -likes R_cutoff0141.beagle.gz -K 2 -o R_cutoff0141_ngsadmix_K2 -P 4

# All samples
FILTERS7="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 23 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 4738"
FILTERS8="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 19 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 3921"
FILTERS9="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 29 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 5882"
FILTERS10="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 35 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 7189"

TODO="-doMajorMinor 1 -doMaf 1 -doCounts 1 -makeMatrix 1 -doIBS 1 -doCov 1 -doGeno 32 -doBcf 1 -doPost 1 -doGlf 2 -dosnpstat 1 -doHWE 1"

angsd -b A.bamlist.txt -GL 1 $FILTERS7 $TODO -P 1 -out A.all -nThreads 20
NGSadmix -likes A.all.beagle.gz -K 2 -o A_ngsadmix_K2 -P 4

angsd -b K.bamlist.txt -GL 1 $FILTERS8 $TODO -P 1 -out K.all -nThreads 20
NGSadmix -likes K.all.beagle.gz -K 2 -o K_ngsadmix_K2 -P 4

angsd -b N.bamlist.txt -GL 1 $FILTERS9 $TODO -P 1 -out N.all -nThreads 20
NGSadmix -likes N.all.beagle.gz -K 2 -o N_ngsadmix_K2 -P 4

angsd -b R.bamlist.txt -GL 1 $FILTERS10 $TODO -P 1 -out R.all -nThreads 20
NGSadmix -likes R.all.beagle.gz -K 2 -o R_ngsadmix_K2 -P 4
...
pcangsd --beagle myresult2_noclones.beagle.gz --admix -o pcangsd_myresult2_noclones --inbreed-samples --selection --threads 12

# Fis of single loci of each population using --inbreed-sites. Need to run each population separately. Filtering follows same procedure as for admix of each site above.
# For MLLs. In the order of AA, AM, KA, KM, NA, NM, RA and RM.
FILTERS1="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 3 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 675"
FILTERS2="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 2 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 507"
FILTERS3="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 12 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2533"
FILTERS4="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 1 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 337"
FILTERS5="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 14 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 3040"
FILTERS6="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 2 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 507"
FILTERS7="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 7 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 1520"
FILTERS8="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 5 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 1182"
# For all samples. Same order as for MLLs.
FILTERS9="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 11 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2287"
FILTERS10="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 12 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2451"
FILTERS11="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 12 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2451"
FILTERS12="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 7 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 1470"
FILTERS13="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 16 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 3268"
FILTERS14="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 13 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2614"
FILTERS15="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 14 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 2941"
FILTERS16="-uniqueOnly 1 -remove_bads 1 -minMapQ 30 -minQ 30 -minInd 21 -snp_pval 1e-5 -sb_pval 1e-5 -skipTriallelic 1 -setMaxDepth 4248"

TODO="-doMajorMinor 1 -doMaf 1 -doCounts 1 -makeMatrix 1 -doIBS 1 -doCov 1 -doGeno 32 -doBcf 1 -doPost 1 -doGlf 2 -dosnpstat 1 -doHWE 1"

# MLLs
angsd -b AA.bamlist.cutoff0141.txt -GL 1 $FILTERS1 $TODO -P 1 -out AA_cutoff0141 -nThreads 20
pcangsd --beagle AA_cutoff0141.beagle.gz -o inbreed_sites_AA_cutoff0141 --inbreed-sites --threads 12
angsd -b AM.bamlist.cutoff0141.txt -GL 1 $FILTERS2 $TODO -P 1 -out AM_cutoff0141 -nThreads 20
pcangsd --beagle AM_cutoff0141.beagle.gz -o inbreed_sites_AM_cutoff0141 --inbreed-sites --threads 12
angsd -b KA.bamlist.cutoff0141.txt -GL 1 $FILTERS3 $TODO -P 1 -out KA_cutoff0141 -nThreads 20
pcangsd --beagle KA_cutoff0141.beagle.gz -o inbreed_sites_KA_cutoff0141 --inbreed-sites --threads 12
angsd -b KM.bamlist.cutoff0141.txt -GL 1 $FILTERS4 $TODO -P 1 -out KM_cutoff0141 -nThreads 20
pcangsd --beagle KM_cutoff0141.beagle.gz -o inbreed_sites_KM_cutoff0141 --inbreed-sites --threads 12
angsd -b NA.bamlist.cutoff0141.txt -GL 1 $FILTERS5 $TODO -P 1 -out NA_cutoff0141 -nThreads 20
pcangsd --beagle NA_cutoff0141.beagle.gz -o inbreed_sites_NA_cutoff0141 --inbreed-sites --threads 12
angsd -b NM.bamlist.cutoff0141.txt -GL 1 $FILTERS6 $TODO -P 1 -out NM_cutoff0141 -nThreads 20
pcangsd --beagle NM_cutoff0141.beagle.gz -o inbreed_sites_NM_cutoff0141 --inbreed-sites --threads 12
angsd -b RA.bamlist.cutoff0141.txt -GL 1 $FILTERS7 $TODO -P 1 -out RA_cutoff0141 -nThreads 20
pcangsd --beagle RA_cutoff0141.beagle.gz -o inbreed_sites_RA_cutoff0141 --inbreed-sites --threads 12
angsd -b RM.bamlist.cutoff0141.txt -GL 1 $FILTERS8 $TODO -P 1 -out RM_cutoff0141 -nThreads 20
pcangsd --beagle RM_cutoff0141.beagle.gz -o inbreed_sites_RM_cutoff0141 --inbreed-sites --threads 12
# All samples
angsd -b AA.bamlist.txt -GL 1 $FILTERS9 $TODO -P 1 -out AA_all -nThreads 20
pcangsd --beagle AA_all.beagle.gz -o inbreed_sites_AA_all --inbreed-sites --threads 12
angsd -b AM.bamlist.txt -GL 1 $FILTERS10 $TODO -P 1 -out AM_all -nThreads 20
pcangsd --beagle AM_all.beagle.gz -o inbreed_sites_AM_all --inbreed-sites --threads 12
angsd -b KA.bamlist.txt -GL 1 $FILTERS11 $TODO -P 1 -out KA_all -nThreads 20
pcangsd --beagle KA_all.beagle.gz -o inbreed_sites_KA_all --inbreed-sites --threads 12
angsd -b KM.bamlist.txt -GL 1 $FILTERS12 $TODO -P 1 -out KM_all -nThreads 20
pcangsd --beagle KM_all.beagle.gz -o inbreed_sites_KM_all --inbreed-sites --threads 12
angsd -b NA.bamlist.txt -GL 1 $FILTERS13 $TODO -P 1 -out NA_all -nThreads 20
pcangsd --beagle NA_all.beagle.gz -o inbreed_sites_NA_all --inbreed-sites --threads 12
angsd -b NM.bamlist.txt -GL 1 $FILTERS14 $TODO -P 1 -out NM_all -nThreads 20
pcangsd --beagle NM_all.beagle.gz -o inbreed_sites_NM_all --inbreed-sites --threads 12
angsd -b RA.bamlist.txt -GL 1 $FILTERS15 $TODO -P 1 -out RA_all -nThreads 20
pcangsd --beagle RA_all.beagle.gz -o inbreed_sites_RA_all --inbreed-sites --threads 12
angsd -b RM.bamlist.txt -GL 1 $FILTERS16 $TODO -P 1 -out RM_all -nThreads 20
pcangsd --beagle RM_all.beagle.gz -o inbreed_sites_RM_all --inbreed-sites --threads 12
...
POPS=("AA.bamlist.cutoff0141" "AM.bamlist.cutoff0141" "KA.bamlist.cutoff0141" "KM.bamlist.cutoff0141" "NA.bamlist.cutoff0141" "NM.bamlist.cutoff0141" "RA.bamlist.cutoff0141" "RM.bamlist.cutoff0141" "AA.bamlist" "AM.bamlist" "KA.bamlist" "KM.bamlist" "NA.bamlist" "NM.bamlist" "RA.bamlist" "RM.bamlist")
...
# Job failed (out of memory). AA, AM, KA and KM were completed. Job failed on step 2 for NA. Will resubmit job with NM, RA and RM. And run step 2-4 as seperate job for only NA.
...
bayescan best_R_cutoff0141 -threads=20' > bayescanFl.sh
